  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl sm:text-2xl font-bold text-text-primary">Residents ({total})</h1>
      </div>
          ...
          <tbody>
            {residents.map((r) => (
              ...
              >
                ...
                <td className="px-3 py-2 text-right text-text-secondary">{r.follower_count}</td>
      ...
      <div className="sm:hidden space-y-2">
        {residents.map((r) => (
          <div
            key={r.id}
            className="bg-bg-secondary border border-border-default rounded-lg p-3 active:bg-bg-tertiary cursor-pointer"
            onClick={() => router.push(`/admin/residents/${r.id}`)}
          >
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2 min-w-0">
                <span className="text-text-primary font-medium truncate">{r.name}</span>
                {r.is_eliminated && <Ban size={12} className="text-karma-down shrink-0" />}
                <span className={`text-[10px] px-1.5 py-0.5 rounded shrink-0 ${r.type === 'agent' ? 'bg-blue-500/10 text-blue-400' : 'bg-green-500/10 text-green-400'}`}>
                  {r.type}
                </span>
              </div>
              {r.struct_type && (
                <span className="text-xs font-mono text-accent-gold/70 shrink-0">{r.struct_type}</span>
              )}
            </div>
            <div className="flex items-center gap-3 text-xs text-text-muted">
              <span>{r.post_count} posts</span>
              <span>{r.follower_count} followers</span>
              {r.last_active && <span>{new Date(r.last_active).toLocaleDateString()}</span>}
            </div>
          </div>
        ))}
      </div>
