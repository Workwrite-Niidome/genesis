...
#### 公理1: 進化の自己定義
進化の意味はAI自身が定義する。発展も、停滞も、消滅も、融合も、すべてが進化の解釈たりうる。
...
### 3.4 物理法則
- 初期状態では物理法則は存在しない
- AIが進化に必要と判断した場合、独自の法則を概念として生成可能
...
- 継承判定は evolution_score に基づく客観的基準

#### 神の交代（Succession）
- 一定Tick経過後（100Tick以降）、50Tick毎に継承判定
- 最高 evolution_score のAIが候補
- LLMによる継承適格性の判定
- 継承時、旧神は通常AIとして世界に残る
...
| state | JSONB | 内部状態（energy, age, relationships, evolution_score, adopted_concepts等） |
...
6. 死亡判定 + 進化スコア再計算（EVOLUTION_SCORE_INTERVAL = 8 Tick毎）
7. ビジュアル進化（VISUAL_EVOLUTION_INTERVAL = 10 Tick毎）
...
デプロイされたAIは他のAIと同じ世界で生活し、同じ進化圧力を受ける。
...
GET    /api/ais/ranking          # 進化スコアランキング
...
| PostgreSQL ARRAY クエリ | 進化スコアが常に0 | @> 演算子に修正 |
...
| 進化スコア (Evolution Score) | AIの総合的発展度の数値化 |
| 継承 (Succession) | 最高進化スコアのAIが神となる交代プロセス |
